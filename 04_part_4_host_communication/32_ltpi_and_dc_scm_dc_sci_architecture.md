...
### 32.1 DC-SCM、DC-SCI、HPM 與 LTPI 的角色
...
### 32.6 LL GPIO 與 NL GPIO

#### 32.6.1 Low-Latency GPIO
...
### 32.9 Data Channel 與 OEM Channel
...
### 32.10 CPLD / FPGA 與 SoC LTPI Controller 責任
...
### 32.12 Device Tree 與 Kernel Binding
...
### 32.13 Power 與 Reset Sequencing
...
### 32.14 BMC Reboot 與 Host Continuity
...
## 32.26 AST2700 LTPI GPIO實作補充

### 32.26.1 AST2700 LTPI Controller能力

AST2700整合兩個獨立LTPI instances。每個instance可提供：

- 最多16個Low-Latency GPIs與16個Low-Latency GPOs。
- 最多128個Normal-Latency GPIs與128個Normal-Latency GPOs。
- LL channel names可表示為`LTPI_ILL[15:0]`與`LTPI_OLL[15:0]`。
- NL channel names可表示為`LTPI_INL[127:0]`與`LTPI_ONL[127:0]`。

此數量是controller capability，不代表platform實際啟用全部channels。最終可用數量仍取決於LTPI capability negotiation、Device Tree、pin / signal mapping、HPM endpoint與board design。

### 32.26.2 TDM與GPIO資料路徑

LTPI使用高速、full-duplex LVDS link上的Time Division Multiplexing承載多個logical channels。GPIO不再以每個signal一條跨connector trace傳送，而是由source endpoint取樣、編入LTPI frame、經link傳輸後在remote endpoint還原。

```text
HPM GPIO / CPLD state
        ↓ sample and classify
LL GPIO or NL GPIO channel
        ↓ LTPI TDM frame
Full-duplex LVDS link
        ↓ decode and synchronize
AST2700 LTPI GPIO controller
        ↓ Linux GPIO subsystem
GPIO line name / gpioget / OpenBMC consumer
```

因此GPIO讀值的有效性同時依賴physical link、training、channel activation、index mapping、polarity與Linux gpiochip mapping。

### 32.26.3 LL與NL GPIO選擇

| 項目 | LL GPIO | NL GPIO |
|---|---|---|
| 每instance GPI | 最多16 | 最多128 |
| 每instance GPO | 最多16 | 最多128 |
| 典型用途 | power、reset、fault、interrupt、boot handshake | board ID、presence、maintenance、一般status |
| 設計重點 | latency、determinism、safe default | channel capacity、update latency、stale state |

Board ID或SKU straps通常可使用NL GPIO，除非其取樣時序位於LTPI link ready之前。若BIOS、CPLD或power sequencer必須在LTPI建立前取得straps，就不能只依賴tunneled GPIO。

### 32.26.4 Board ID讀取案例

以`FM_BMC_BOARD_SKU_ID0_N`為例，平台mapping將其對應至`BMC_GPIO18`。若該SoC revision的LTPI GPIO control register定義如下：

```text
LTPI GPIO register base = 0x14C34C00
GPIO index             = 18
Register stride        = 4 bytes
Register address       = 0x14C34C00 + (18 × 4)
                       = 0x14C34C48
Input value bit        = bit 13
```

可在核准的實驗環境中直接讀取：

```sh
val=$(devmem 0x14c34c48 32)
echo $(( (val >> 13) & 1 ))
```

`devmem`僅適合bring-up或register cross-check。正式軟體應使用Linux GPIO character device與line name：

```sh
gpiofind FM_BMC_BOARD_SKU_ID0_N
gpioget "$(gpiofind FM_BMC_BOARD_SKU_ID0_N)"
```

實際`gpioget`語法會因libgpiod major version而不同，應先執行`gpioget --help`。不要在未確認register map、SoC revision與ownership前使用`devmem`寫入LTPI GPIO registers。

### 32.26.5 Board SKU Mapping範例

範例平台中，SKU straps可對應至下列Linux GPIO lines：

| Strap | Signal Name | GPIO Line | Example Value |
|---|---|---|---:|
| SKU_ID0 | FM_BMC_BOARD_SKU_ID0_N | gpiochip5 line 36 | 0 |
| SKU_ID1 | FM_BMC_BOARD_SKU_ID1_N | gpiochip5 line 38 | 0 |
| SKU_ID2 | FM_BMC_BOARD_SKU_ID2_N | gpiochip5 line 40 | 0 |
| SKU_ID3 | FM_BMC_BOARD_SKU_ID3_N | gpiochip5 line 42 | 0 |
| SKU_ID4 | FM_BMC_BOARD_SKU_ID4_N | gpiochip5 line 44 | 0 |
| SKU_ID5 | FM_BMC_BOARD_SKU_ID5_N | gpiochip5 line 46 | 0 |
| SKU_ID6 | FM_BMC_BOARD_SKU_ID6_N | gpiochip5 line 48 | 0 |
| SKU_ID7 | FM_BMC_BOARD_SKU_ID7_N | gpiochip5 line 50 | 1 |

以上line numbers與values是case-study資料，不可直接複製到其他platform。Production mapping應以DTS line names、schematic、LTPI channel map與runtime `gpioinfo`共同確認。

### 32.26.6 三層交叉驗證

每一個LTPI GPIO至少做三層比對：

1. **Register layer**：依datasheet確認instance、GPIO index、offset與input bit。
2. **Linux GPIO layer**：使用`gpioinfo`、`gpiofind`、`gpioget`確認chip、offset、direction、active-low與consumer。
3. **Platform semantic layer**：確認signal在schematic的active polarity、來源、power domain與SKU bit weight。

若三層不一致，優先檢查DTS line-name排列、LL / NL分類、instance與remote endpoint channel mapping，而不是直接修改application。

### 32.26.7 實務限制與驗收

- GPIO base address與bit definition依SoC revision及datasheet，必須版本化。
- `gpiochipN`編號不保證跨kernel或probe order穩定，application應使用line name或穩定lookup。
- Active-low signal的raw electrical value與logical asserted value必須區分。
- Link retrain期間的GPIO值應標示invalid / stale，不能把暫態預設值解讀成真實SKU或fault。
- 驗證每一個SKU bit的0 / 1組合，並與CPLD register、FRU / inventory結果交叉比對。
- 測試BMC reset、LTPI endpoint reset、HPM replacement與link loss後是否重新讀取Board ID。
