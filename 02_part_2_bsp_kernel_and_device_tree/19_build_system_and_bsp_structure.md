...
## 19.1.7 BMC Porting 時 Yocto 需要優先確認的檔案
...
## 19.2.6 Metadata, Recipe 與 Layer
...
## 19.3 在 Docker 中建立 Yocto 專案並建置完整映像
...
## 19.4.4 完整開發循環: Modify → Build → Test
...
## 19.4.6 clean / cleansstate / cleanall 何時使用?
...
## 19.5.5 動手做: 用 . bbappend 修改 cmake-native 行為
...
## 19.5.7 多個 . bbappend 的順序與 layer priority
...
## 19.6 使用 devtool 修改原始碼並產出補丁(Patch)
...
## 19.6.5 產出 patch: update-recipe 與 finish
...
## 19.6.6 update-recipe, finish, 手寫 . bbappend 的選擇
...
## 19.6.8 deploy-target: 部署到 live BMC 測試
...
## 19.7.10 Package split: 把檔案分成多個 package
...
## 19.7.12 devtool add: 新增套件並進入開發模式
...
## 19.7.14 BMC / OpenBMC recipe 檢查重點
...
## 19.8 進階混合開發: devtool modify / update-recipe / finish
...
## 19.8.1 `update-recipe` 和 `finish` 的差異
...
## 19.9 OpenBMC 新 Machine Layer 與 DTS Bring-up 系統化流程
...
### 19.9.5 階段 4: 確認 BBLAYERS 載入真正 layer root
...
### 19.9.6 階段 5: 先沿用 EVB DTB, 確認 image build flow
...
### 19.9.7 階段 6: 導入 Linux kernel DTS
